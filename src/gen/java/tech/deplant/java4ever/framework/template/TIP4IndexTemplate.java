...
public record TIP4IndexTemplate(ContractAbi abi, Tvc tvc) implements Template {
  ...
  public DeployHandle<TIP4IndexContract> prepareDeploy(Sdk sdk, Credentials credentials,
      Address _nft, Address collection) {
    Map<String, Object> initialDataFields = Map.of("_nft", _nft);
    Map<String, Object> params = Map.of("collection", collection);
    return new DeployHandle<TIP4IndexContract>(TIP4IndexContract.class, sdk, abi(), tvc(), sdk.clientConfig().abi().workchain(), credentials, initialDataFields, params, null);
  }
}
